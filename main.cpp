...
void mainLoop(const DWORD pid, const HANDLE driver) {
		...
		if (const std::uintptr_t client = get_module_base(pid, L"client.dll"); client != 0) {
			...
			while (true) {
				...
				const auto force_jump = driver::read_memory<DWORD>(
					driver, client + client_dll::dwForceJump
				);

				if (space_pressed && in_air) {
					std::cout << "Space pressed and in air. Writing 65537 to ForceJump.\n";
					Sleep(5);
					driver::write_memory(driver, client + client_dll::dwForceJump, 65537);
				}
				else if (space_pressed && !in_air) {
					std::cout << "Space pressed and not in air. Writing 256 to ForceJump.\n";
					driver::write_memory(driver, client + client_dll::dwForceJump, 256);
				}
				else if (!space_pressed && force_jump == 65537) {
					std::cout << "Space not pressed and ForceJump is 65537. Writing 256 to ForceJump.\n";
					driver::write_memory(driver, client + client_dll::dwForceJump, 256);
				}
				std::this_thread::sleep_for(std::chrono::milliseconds(1)); // Sleep for a short time to reduce CPU usage
			}
		}
	}
	CloseHandle(driver);
...
int main() {
	...
	const HANDLE driver = CreateFile(L"\\\\.\\SexyDriver", GENERIC_READ, 0, nullptr, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, nullptr);
	...
	std::thread mainThread(mainLoop, pid, driver);
	mainThread.detach(); // Detach the thread to run independently
